-- ============================================================================
-- MASTERKEY COMMUNICATIONS SYSTEM
-- ============================================================================
-- Comprehensive communications system for property management
-- Features: Multi-channel messaging, templates, automation, analytics
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS communications (
  ...
  failed_reason TEXT,
  metadata JSONB, -- attachments, links, formatting, etc.
...
CREATE INDEX IF NOT EXISTS idx_comms_recipients ON communications USING GIN(recipient_ids);
...
CREATE TABLE IF NOT EXISTS message_templates (
  ...
  subject VARCHAR(255),
  body TEXT NOT NULL,
  merge_fields TEXT[], -- {{tenant_name}}, {{unit_number}}, etc.
...
CREATE TABLE IF NOT EXISTS communication_workflows (
  ...
  trigger_conditions JSONB, -- days before/after, specific dates, etc.
  steps JSONB, -- array of workflow steps with delays, channels, templates
...
CREATE TABLE IF NOT EXISTS communication_preferences (
  ...
  language VARCHAR(10) DEFAULT 'en',
  notification_settings JSONB, -- granular preferences
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS quick_replies (
  ...
  shortcut VARCHAR(50) NOT NULL, -- e.g., "/paid", "/working"
  text TEXT NOT NULL,
...
DROP POLICY IF EXISTS comms_select_policy ON communications;
CREATE POLICY comms_select_policy ON communications
...
DROP POLICY IF EXISTS comms_insert_policy ON communications;
CREATE POLICY comms_insert_policy ON communications
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Allow users to update their own messages (mark as read, etc.)
DROP POLICY IF EXISTS comms_update_policy ON communications;
CREATE POLICY comms_update_policy ON communications
...
INSERT INTO message_templates (name, category, subject, body, merge_fields, tone, is_system_template) VALUES
...
We're thrilled to have you as part of the {{property_name}} community!
...
If you need anything or have questions, we're here to help!
...
We hope you've enjoyed living at {{property_name}}! Your current lease for unit {{unit_number}} expires on {{lease_end_date}}.

We'd love to have you stay with us! Here's your renewal offer:
...
INSERT INTO communication_workflows (name, description, trigger_type, trigger_conditions, steps, is_active) VALUES
('Late Rent Follow-up',
...
DO $$ BEGIN
  RAISE NOTICE '====================================================';
  RAISE NOTICE 'MasterKey Communications System Created!';
  RAISE NOTICE '====================================================';
  RAISE NOTICE 'Tables Created:';
  RAISE NOTICE '  • conversation_threads';
  RAISE NOTICE '  • communications';
  RAISE NOTICE '  • message_templates (5 samples)';
  RAISE NOTICE '  • communication_workflows (1 sample)';
  RAISE NOTICE '  • communication_preferences';
  RAISE NOTICE '  • delivery_tracking';
  RAISE NOTICE '  • quick_replies';
  RAISE NOTICE '';
  RAISE NOTICE 'Features Ready:';
  RAISE NOTICE '  ✓ Multi-channel messaging (Email, SMS, Portal, Push)';
  RAISE NOTICE '  ✓ Conversation threading';
  RAISE NOTICE '  ✓ Message templates';
